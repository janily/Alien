...
var dom = {}, InvalidFinder = Error, macros, rules, tokenMap,
  ...
  alien.addDetectionTest('classList', function() {
  ...
  alien.addDetectionTest('querySelectorAll', function() {
    var div = document.createElement('div');
  ...
  dom.get = function(selector) {
    ...
    return alien.toArray(alien.detect('querySelectorAll') ?
  ...
  alien.addDetectionTest('getAttribute', function() {
    var div = document.createElement('div');
  ...
  dom.attr = function(element, attribute, value) {
    if (typeof value === 'undefined') {
      return alien.detect('getAttribute') ?
  ...
  // Chained API
  alien.init(function(arg) {
    if (typeof arg === 'string' || typeof arg === 'undefined') {
      // CSS selector
      return alien.domChain.init(arg);
    }
  });
  ...
  alien.domChain.init.prototype = alien.domChain;

  /**
    * Enumerable methods can be chained with DOM calls:
    *
    *       alien('p').each(function(element) {
    *         console.log(element);
    *       });
    *
    */
  if (typeof alien.enumerable !== 'undefined') {
    alien.domChain['values'] = function() {
      return this.elements;
    };

    alien.enumerable.each(alien.chainableMethods, function(methodName) {
      alien.domChain[methodName] = function(fn) {
        var elements = alien.enumerable[methodName](this, fn),
            ret = alien.domChain;
        this.elements = elements;
        ret.elements = elements;
        ret.selector = this.selector;
